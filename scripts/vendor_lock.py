...
spytial vendors three files out of the spytial-core npm tarball -- the language
manifest, the spec schema, and the tier-2 evaluator bundle -- and separately
pins a version in ``spytial/core_assets.py`` that decides which browser bundle
is loaded from the CDN. All four have to be the same release. A spec validated
against one version of the language and rendered by another is exactly the
silent mismatch the vendoring exists to prevent.

The manifest and the schema each state their own version, so they can be
checked directly. The evaluator bundle does not: it is a minified build with no
version string anywhere in it, so nothing about the file itself says which
release produced it. This lockfile is what makes that one checkable -- it
records the version and a content hash at vendor time, written by
...
catches a pin moved by hand without re-running the update script.
"""
...
def capture(version=None):
    ...
    return {
        "_comment": (
            ...
            "Records which spytial-core release each vendored file came from, so "
            "the evaluator bundle -- which carries no version string of its own -- "
            "can still be checked against the pin in core_assets.py."
        ),
