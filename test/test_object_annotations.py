...
def test_sub_object_annotations_persist_on_composition():
    ...
    # Should have at least 1 constraint (fruits and numbers have the same constraint,
    # so deduplication results in a single constraint in the final spec)
    # The key point is that sub-object annotations are being collected
    constraint_count = len(collected_decorators["constraints"])
    assert (
        constraint_count >= 1
    ), f"Expected at least 1 constraint, got {constraint_count}"

    # Verify that the constraint is the group constraint we applied
    assert any(
        "group" in c for c in collected_decorators["constraints"]
    ), "Expected to find group constraint in collected decorators"
