...
def test_relationalizer_inheritance_requirement():
    ...
    with pytest.raises(
        TypeError, match="Relationalizer must inherit from RelationalizerBase"
    ):
...
def test_basic_relationalizer_implementation():
    ...
    try:
        ...
        @relationalizer(
            priority=100
        )  # Custom relationalizers should use priority >= 100
...
def test_built_in_relationalizers_work():
    ...
    assert (
        len(data_instance["atoms"]) >= 8
    )  # At least one for each element plus the root dict
...
def test_registry_management():
    ...
    try:
        ...
        not_found_relationalizer = RelationalizerRegistry.find_relationalizer(
            "not_test"
        )
...
if __name__ == "__main__":
    ...
    if test_relationalizer_inheritance_requirement_manual():
        ...
        print(
            "✗ test_relationalizer_inheritance_requirement: Should have raised TypeError"
        )
