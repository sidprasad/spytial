...
def test_relation_class_nary_functionality():
    ...
    with pytest.raises(ValueError, match="to_tuple\\(\\) only supports binary relations"):
        rel.to_tuple()
...
def test_relation_class_validation():
    ...
    with pytest.raises(ValueError, match="Either \\(source_id, target_id\\) or atoms must be provided"):
        ...
        class MixedRelationRelationalizer(RelationalizerBase):
            ...
            def relationalize(self, obj: Any, walker_func) -> Tuple[List[Atom], List[Relation]]:
                ...
                relations = [
                    ...
                    # Quaternary relation
                    Relation.from_atoms("quaternary_rel", [obj_id, child1_id, child2_id, child3_id])
                ]
...
def test_nary_relations_visualization():
    ...
    try:
        ...
        class SimpleNAryRelationalizer(RelationalizerBase):
            ...
            def can_handle(self, obj: Any) -> bool:
                return isinstance(obj, tuple) and len(obj) >= 3 and obj[0] == 'nary_test'
            
            def relationalize(self, obj: Any, walker_func) -> Tuple[List[Atom], List[Relation]]:
                obj_id = walker_func._get_id(obj)
                atom = Atom(id=obj_id, type="nary_test", label=f"N-ary Test ({len(obj)} elements)")
