...
class CourseRelationalizer(RelationalizerBase):
    ...
    def relationalize(self, obj: Any, walker_func) -> Tuple[List[Atom], List[Relation]]:
        ...
        instructor_course_rel = Relation.binary(
            "instructs", instructor_id, course_id
        )
        course_semester_rel = Relation.binary(
            "offered_in", course_id, semester_id
        )
        
        return [course_atom], [teaches_relation, instructor_course_rel, course_semester_rel]
...
class MeetingRelationalizer(RelationalizerBase):
    ...
    def can_handle(self, obj: Any) -> bool:
        return (isinstance(obj, dict) and 
                'meeting_title' in obj and 
                'participants' in obj and
                isinstance(obj['participants'], list))
    ...
    def relationalize(self, obj: Any, walker_func) -> Tuple[List[Atom], List[Relation]]:
        ...
        meeting_atom = Atom(
            id=meeting_id,
            type="meeting",
            label=f"Meeting: {obj['meeting_title']}"
        )
        
        # Create atoms for all participants
        participant_ids = [walker_func(participant) for participant in obj['participants']]
...
def demonstrate_nary_relations():
    ...
    quaternary_rel = Relation.from_atoms("transaction", ["sender", "receiver", "amount", "timestamp"])
    
    print(f"   Binary relation: {binary_rel.arity()}-ary - {binary_rel.to_tuple()}")
    print(f"   Ternary relation: {ternary_rel.arity()}-ary - {ternary_rel.to_atoms_tuple()}")
    print(f"   Quaternary relation: {quaternary_rel.arity()}-ary - {quaternary_rel.to_atoms_tuple()}")
